...
async def main():
    ...
    wallet_address = '0xYourBaseSepoliaWalletAddress'  # <-- Replace with your address
    await page.type('input[type="text"]', wallet_address)

    # Select Base Sepolia faucet (update selector if needed)
    # The label's "for" attribute might be like "baseSepolia"
    await page.click('label[for="baseSepolia"]')
